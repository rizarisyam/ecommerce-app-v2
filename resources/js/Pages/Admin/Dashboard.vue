          ...
          <div class="relative flex-1 flex flex-col max-w-xs w-full pt-5 pb-4 bg-white">
            ...
            <div class="mt-5 flex-1 h-0 overflow-y-auto">
              <nav class="px-2 space-y-1">
                <a v-for="item in navigation" :key="item.name" :href="item.href.includes('#') ? item.href : item.href" :class="[item.current ? 'bg-gray-100 text-gray-900' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900', 'group flex items-center px-2 py-2 text-base font-medium rounded-md']">
      ...
      <div class="flex flex-col w-64">
        ...
        <div class="flex flex-col flex-grow border-r border-gray-200 pt-5 pb-4 bg-white overflow-y-auto">
          <div class="flex items-center flex-shrink-0 px-4">
            <img class="h-8 w-auto" src="/img/logos/workflow-logo-indigo-600-mark-gray-800-text.svg" alt="Workflow" />
          </div>
...
const navigation = [
  { name: 'Dashboard', href: '#', icon: HomeIcon, current: route().current('dashboard') },
  { name: 'Categories', href: route("product-categories.index"), icon: UsersIcon, current: route().current('product-categories.*') },
  { name: 'Discounts', href: route("product-discounts.index"), icon: FolderIcon, current: route().current('product-discounts.*') },
  { name: 'Products', href: route("products.index"), icon: CalendarIcon, current: route().current('products.*') },
